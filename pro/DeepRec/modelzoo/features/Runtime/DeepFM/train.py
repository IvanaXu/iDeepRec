...
def get_arg_parser():
    ...
    parser.add_argument('--dense_layer_partitioner',
                        ...
                        type=int,
                        default=0)
    parser.add_argument('--use_cost_model_executor',
                        help='Use cost model executor or not.',
                        type=bool,
                        default=False)
    parser.add_argument('--use_thread_local_executor',
                        help='Use thread local executor or not.',
                        type=bool,
                        default=False)
    return parser
...
def main(tf_config=None, server=None):
    ...
    if args.use_cost_model_executor:
        sess_config.executor_policy = tf.ExecutorPolicy.USE_COST_MODEL_EXECUTOR
    if args.use_thread_local_executor:
    ...
    if tf_config:
        ...
        with tf.train.MonitoredTrainingSession(
            ...
            while not sess.should_stop():
                if args.timeline > 0 and curr_step % args.timeline == 0:
                    ...
                    with open(os.path.join(checkpoint_dir,
                                           'timeline-%d.json' % curr_step), 'w') as f:
            ...
            for _in in range(0, train_steps):
                if args.save_steps > 0 and (_in % args.save_steps == 0
                    ...
                    _, train_loss, events = sess.run(
                        [model.train_op, model.loss, merged], options=options1)
                    ...
                    _, train_loss = sess.run([model.train_op, model.loss], options=options1)
